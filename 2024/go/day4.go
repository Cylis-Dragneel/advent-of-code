...
func day4_1(input string) {
		...
		for col := 0; col < len(grid[row]); col++ {
			for _, dir := range directions {
				...
				for charIndex := 0; charIndex < len(word); charIndex++ {
					...
					if rowOffset < 0 || rowOffset >= len(grid) || colOffset < 0 ||
						colOffset >= len(grid[row]) {
						isXMAS = false
						break
					}
	...
	fmt.Println("Day 4 Part 1 Output is", output)
}
...
func day4_2(input string) {
	...
	for row := 0; row < len(grid); row++ {
		for col := 0; col < len(grid[row]); col++ {
			...
			for _, dir := range directions {
				...
				if row1Index < 0 || row1Index >= len(grid) || col1Index < 0 ||
					col1Index >= len(grid[row]) ||
					row2Index < 0 ||
					row2Index >= len(grid) ||
					col2Index < 0 ||
					col2Index >= len(grid[row]) {
					isMAS = false
					break
				}
	...
	fmt.Println("Day 4 Part 2 Output is", output)
}
